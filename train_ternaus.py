...
parser.add_argument("--crop-dim", type=int, default=128,
...
def run_epoch(model, epoch, data_loader, device, mode="train", writer=None):
    ...
    for image, mask, label in data_loader:
        ...
        margin = args.crop_dim // 10
        if args.crop_end:
            true_mask = true_mask[:,margin:-margin,margin:-margin]
            out_mask = out_mask[:,:,margin:-margin,margin:-margin]
...
if __name__ == "__main__":
    ...
    if args.train_per_class:
        ...
        model = ternausnet2.TernausNetV2(num_classes = 9)

        state = torch.load('/mnt/kvaditya/EyeInTheSky/deepglobe_buildings.pt')
        ...
        # model.conv1_correct.conv1_correct.weight[:,3,:,:] = 0
        # model.conv1_correct.conv1_correct.weight.requires_grad = False
